...
const App = () => {
  ...
  const addName = (event) => {
    ...
    if (isNumberPresent && oldPerson){
    ...
    else if (personToChange && !isNumberPresent){
      window.confirm(`${newName} is already added to phonebook, replace the old number with a new one?`)&&
      phonebook.update(oldPerson, newPerson).then((updatedPerson) => {
        setPersons(persons.map((person)=>
        person.name !== oldPerson.name ? person : updatedPerson
        ))
        setNotification(
      ...
      .catch(error => {
        setNotification(
          `Information of ${newName} has encounter some issues`
        )
        setTimeout(() => {
          setNotification(null)
        }, 5000)
